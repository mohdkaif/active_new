 ...
 if($_SESSION['adminUName']=='')
 {
  ...
  if(isset($_POST['submit'])){
   if($_FILES['about_image']['name']<>''){
    $res='';
   $ifileext=explode('.',$_FILES['about_image']['name']);
   $ifileext1=$ifileext[count($ifileext)-1];
   if($ifileext1=='jpg' or $ifileext1=='jpeg' or $ifileext1=='png')
   {
    $image_file=$_FILES['about_image']['name'];
   $itarget2 = "images/";
   $itarget2 = $itarget2.$image_file;
   $query=$mysqli->query("SELECT * FROM `about` WHERE `about_id`='$id'");
   if($query->num_rows > 0)
   {
   ...
   move_uploaded_file($_FILES['about_image']['tmp_name'], $itarget2);
   $target_file = $itarget2.basename($_FILES["about_image"]["name"]);
   $res=$mysqli->query("UPDATE `about` SET `about_image`='".$image_file."',`about_heading`='".addslashes($_POST['about_heading'])."',`about_description`='".addslashes($_POST['about_description'])."' WHERE `about_id`='$id'");
  }     
  if($res)
  {
  $msg='<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">X</button><strong>Well done!</strong> Image succesfully Saved</div>';
   ...
   elseif($_POST['about_heading']<>'')
   {
  $res=$mysqli->query("UPDATE `about` SET `about_heading`='".addslashes($_POST['about_heading'])."',`about_description`='".addslashes($_POST['about_description'])."' WHERE `about_id`='$id'");
            ...
            <div class="card-body">
                <form  method="post" enctype="multipart/form-data">
                      <div class="form-group">
                        ...
                        <div class="col-sm-10">
                         <input type="file" name="about_image" onChange="readURL(this,1);" required">
                         <p class="help-block">You can upload only png and jpg or jpeg.</p>
                <img id="image1" height="200px;" width="250px;" src="images/<?php echo $about_res['about_image'];?>">
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="col-sm-2 control-label">Heading</label>
                        <div class="col-sm-10">
                            <input type="text" name="about_heading" value="<?php echo $about_res['about_heading'];?>" class="form-control">
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="col-sm-2 control-label">Description</label>
                        <div class="col-sm-10">
                           <textarea id="editor1" name="about_description" rows="10" cols="80"><?php echo $about_res['about_description'];?></textarea>
                        </div>
                      </div>
